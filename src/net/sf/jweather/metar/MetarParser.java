...
public class MetarParser {
	...
	public static Metar parseReport(String metarString)
		...
		try {
			...
			log.fine("MetarParser: instantiated");

			if (metarString == null) {
				throw new MetarParseException("empty metar data");
			}

			log.fine("MetarParser: raw: " + metarString);
			...
			try {
				...
				log.severe("MetarParser: error spliting metar data on whitespace: "
						+ e);
			...
			log.fine("MetarParser: have '" + numTokens + "' tokens");
			...
			log.fine("MetarParser: stationID: " + metar.getStationID());

			log.fine("MetarParser: processing ((String)tokens.get(" + index
			...
			if (((String) tokens.get(index)).endsWith("Z")) {
				...
				log.fine("MetarParser: date: " + metar.getDate());
			} else {
				log.fine("MetarParser: date: no date found");
			...
			if (((String) tokens.get(index))
				...
				log.fine("MetarParser: report modifier: "
						+ metar.getReportModifier());
			} else {
				log.fine("MetarParser: no report modifier");
			}
			...
			if (temp.endsWith("KT") || temp.endsWith("KTS")
				...
				if (temp.endsWith("KT") || temp.endsWith("KTS")
					...
					log.fine("MetarParser: wind speed in knots");
					windInKnots = true;
				} else {
					log.fine("MetarParser: wind speed in meters per second");
				}

				if (!((String) tokens.get(index)).substring(0, 3).equals("VRB")) {
					// we have gusts
					Integer windDirection = new Integer(
							((String) tokens.get(index)).substring(0, 3));
					metar.setWindDirection(windDirection);
				} else {
					log.fine("MetarParser: variable wind direction <= 6 knots");
				...
				// 5th position
				if (!metar.getWindDirectionIsVariable() && temp.length() >= 6
						&& Character.isDigit(temp.charAt(5))) {
					// have three-digit wind speed
					log.fine("MetarParser: have three-digit wind speed");
					if (windInKnots) {
					...
					log.fine("MetarParser: have two-digit wind speed");
					if (windInKnots) {
						metar.setWindSpeed(new Float(((String) tokens
				...
				if (temp.length() > pos && temp.charAt(pos) == 'G') {
					// we have wind gusts
					log.fine("MetarParser: wind gusts");
					pos++;
					...
					try {
						...
						if (temp.matches("\\d")) {
							// have three-digit wind speed
							log.severe("MetarParser: have three-digit wind speed");
							if (windInKnots) {
						...
						log.severe("MetarParser: error matching wind gusts: "
								+ e);
					}
				} else {
					// we don't have gusts
					log.fine("MetarParser: no gusts");
				}

				log.fine("MetarParser: wind direction: "
						+ metar.getWindDirection());
				if (windInKnots) {
					log.fine("MetarParser: wind speed: "
				...
				try {
					if (temp.matches(".*\\d\\d\\dV\\d\\d\\d")) {
						...
						log.fine("MetarParser: variable wind direction min: "
								+ metar.getWindDirectionMin());
						log.fine("MetarParser: variable wind direction max: "
					...
					log.severe("MetarParser: error matching variable wind speed: "
							+ e);
				}
			} else {
				// unexpected token...should have been wind speed
				log.fine("MetarParser: wind speed: not found");
			}
			...
			if (((String) tokens.get(index)).equals(MetarConstants.METAR_CAVOK)) {
				...
				log.fine("MetarParser: visibility");
				...
				if (token.startsWith("M")) {
					log.fine("MetarParser: visibility: less than");
					isLessThan = true;
				...
				if (!fraction.equals("")) {
					...
					try {
						...
						log.severe("MetarParser: error spliting fraction on /: "
								+ e);
						throw new MetarParseException(
				...
				log.fine("MetarParser: visibility: " + metar.getVisibility()
						+ " M");
			} else {
				String token = (String) tokens.get(index);
				boolean isLessThan = false;
				...
				if (Pattern.matches("^M?\\d+(N|NE|E|SE|S|SW|W|NW)?$", token)) {
					...
					log.fine("MetarParser: visibility: not found");
				}
			}
			...
			while (((String) tokens.get(index)).startsWith("R")) {
				...
				log.fine("MetarParser: found RVR");
				...
				log.fine("MetarParser: RVR runway number: "
				...
				if (((String) tokens.get(index)).charAt(pos) != '/') {
					...
					log.fine("MetarParser: RVR runway approach direction: "
				...
				case 'M': // above 6000ft
					runwayVisualRange.setReportableModifier(((String) tokens
							.get(index)).charAt(pos));
					log.fine("MetarParser: RVR modifier: "
							+ ((String) tokens.get(index)).charAt(pos));
					pos++;
				}
				runwayVisualRange.setLowestReportable(new Integer(
						...
						.intValue());
				log.fine("MetarParser: RVR lowest reportable: "
				...
				if (((String) tokens.get(index)).charAt(pos) == 'V') {
					...
					log.fine("MetarParser: RVR highest reportable: "
			...
			while (((String) tokens.get(index))
				...
				log.fine("MetarParser: found weather groups");
				...
				if (((String) tokens.get(index))
					...
					log.fine("MetarParser: weather group: intensity: "
							+ ((String) tokens.get(index)).charAt(0));
					pos++;
				} else {
					log.fine("MetarParser: weather group: intensity: moderate");
				}
				...
				if (((String) tokens.get(index)).substring(pos, pos + 2)
					...
					weatherCondition.setDescriptor(((String) tokens.get(index))
							.substring(pos, pos + 2));
					log.fine("MetarParser: weather group: descriptor: "
							+ ((String) tokens.get(index)).substring(pos,
									pos + 2));
					pos += 2;
				} else {
					log.fine("MetarParser: weather group: descriptor: no descriptor");
				}
				...
				if (((String) tokens.get(index)).substring(pos, pos + 2)
					...
					weatherCondition.setPhenomena(((String) tokens.get(index))
							.substring(pos, pos + 2));
					log.fine("MetarParser: weather group: phenomena: "
							+ ((String) tokens.get(index)).substring(pos,
									pos + 2));
					metar.addWeatherCondition(weatherCondition);
					log.fine("MetarParser: "
							+ weatherCondition.getNaturalLanguageString());
				} else {
					log.fine("MetarParser: weather group: no phenomena");
				}
			...
			while (((String) tokens.get(index))
				...
				log.fine("MetarParser: found sky conditions");
				...
				if (((String) tokens.get(index))
					...
					skyCondition.setContraction(((String) tokens.get(index))
							.substring(0, 3));
					log.fine("MetarParser: sky condition: contraction: "
							+ ((String) tokens.get(index)).substring(0, 3));
					skyCondition.setHeight(new Integer(((String) tokens
							.get(index)).substring(3, 6)).intValue());
					log.fine("MetarParser: sky condition: height: "
					...
					if (((String) tokens.get(index)).length() > 6) {
						...
						log.fine("MetarParser: sky condition: modifier: "
					...
					skyCondition.setContraction(((String) tokens.get(index))
							.substring(0, 3));
					log.fine("MetarParser: sky condition: clear: "
							+ ((String) tokens.get(index)).substring(0, 3));
				} else if (((String) tokens.get(index))
						.startsWith(MetarConstants.METAR_VERTICAL_VISIBILITY)) {
					skyCondition.setContraction(((String) tokens.get(index))
					...
					skyCondition.setContraction(((String) tokens.get(index))
							.substring(0, 3));
					log.fine("MetarParser: sky condition: No Significant Clouds: "
							+ ((String) tokens.get(index)).substring(0, 3));
				} else {
					log.fine("MetarParser: sky condition: couldn't determine which");
				}
			...
			if (((String) tokens.get(index)).indexOf("/") != -1) {
				log.fine("MetarParser: found temperature");
				...
				try {
					...
					log.severe("MetarParser: error spliting temperature on /: "
							+ e);
					throw new MetarParseException(
				...
				if (temperature != null)
					log.fine("MetarParser: temperature: " + temperature
							+ " C, " + (temperature.floatValue() * 9 / 5 + 32)
							+ " F");
				...
				if (temps.size() < 2 || ((String) temps.get(1)).length() == 0) {
					...
					dewPoint = new Float(
							((String) temps.get(1)).substring(1, 3));
				...
				if (dewPoint != null)
					log.fine("MetarParser: dew point: " + dewPoint + " C, "
				...
				log.fine("MetarParser: temperature/dew point not found");
				metar.setTemperature(null);
				metar.setDewPoint(null);
			}
			...
			// Measured in hecto Pascal (HPa), 1 Hpa = 1 mB(millibar)
				...
				log.fine("MetarParser: pressure: " + pressure + " hPa");
				...
				log.fine("MetarParser: pressure not found");
			}
			...
			if (!((String) tokens.get(index))
				...
				// we have no remarks
				log.fine("MetarParser: we have no remarks");
			} else {
				log.fine("MetarParser: we have remarks");
				index++;
			}
			...
			// ---------------------------------------------------------------
			if (index < numTokens
					&& MetarConstants.METAR_BECOMING
							.equalsIgnoreCase((String) tokens.get(index))) {
				StringBuffer sb = new StringBuffer();

				log.fine("MetarParser: processing \"BECMG\" section");
				...
				while (index < numTokens
					...
					log.fine("MetarParser: processing ((String)tokens.get("
			...
			while (index < numTokens) {
				...
				try {
					...
					if (temp.matches("T\\d{8}")) {
						log.fine("MetarParser: found detailed temp: "
								+ ((String) tokens.get(index)));
						// we have a sub-zero temperature
						Float temperaturePrecise = new Float(
								((String) tokens.get(index)).substring(2, 5));
						if (((String) tokens.get(index)).charAt(1) == '1') {
							temperaturePrecise = new Float(
									temperaturePrecise.floatValue()
											- temperaturePrecise.floatValue()
											* 2); // negate
						}
						// it is in tenths
						temperaturePrecise = new Float(
								temperaturePrecise.floatValue() / 10);
						metar.setTemperaturePrecise(temperaturePrecise);

						// we have a sub-zero dew point
						Float dewPointPrecise = new Float(
								((String) tokens.get(index)).substring(6, 9));
						if (((String) tokens.get(index)).charAt(5) == '1') {
							dewPointPrecise = new Float(
									dewPointPrecise.floatValue()
											- dewPointPrecise.floatValue() * 2); // negate
						}
						// it is in tenths
						dewPointPrecise = new Float(
								dewPointPrecise.floatValue() / 10);
						metar.setDewPointPrecise(dewPointPrecise);

						log.fine("MetarParser: temperature precise: "
								+ temperaturePrecise
								+ " C, "
								+ (temperaturePrecise.floatValue() * 9 / 5 + 32)
								+ " F");
						log.fine("MetarParser: dew point precise: "
								+ dewPointPrecise + " C, "
					...
					log.severe("MetarParser: error matching additional remarks: "
							+ e);
				}
			...
			log.fine("MetarParser: done processing metar data");
